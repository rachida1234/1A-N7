...
void create(string *chaine_dest, char *chaine_src){
    ...
    chaine_dest->str = NULL;
    chaine_dest->N = 0;
    int i = 0;
    while (chaine_src[i]!='\0'){
        chaine_dest->str=realloc(chaine_dest->str,(i+1)*taille);
...
void add(string *chaine, char c) {
    int taille = sizeof(char);
    chaine->str=realloc(chaine->str,taille + sizeof(chaine->str));
    (*chaine).N=(*chaine).N+1;
    chaine->str[length(*chaine)-1] = c;
    chaine->str[length(*chaine)]='\0';
}
