...
for imat = 1:4
    ...
    for i = 1:length(p)
        ...
        % Calculer les valeurs propres nécessaires avec subspace_iter_v3
        [ W, V, n_ev, it, itv, flag ] = subspace_iter_v3( A, m, percentage, p(i), eps, maxit);

        % Comparer le temps courant au temps retenu
        t_v1 = cputime-t_v1;
        temps_IT(i)=t_v1;

        % Récupérer le nombre d'itérations qui ont été nécessaires pour
        % cette valeur de p
        nbr_IT(i)=it;
